...
/// Decodes a camera RAW buffer with the canonical RAW Alchemy parameters.
///
/// # Errors
///
...
pub fn decode(bytes: &[u8], half_size: bool) -> Result<DecodedImage, String> {
    ...
    let status =
        unsafe { alchemy_libraw_decode(bytes.as_ptr(), bytes.len(), half_size, &raw mut output) };
    if status != 0 {
    ...
    unsafe { alchemy_libraw_free(output.pixels) };
...
pub fn version() -> String {
    ...
    unsafe { CStr::from_ptr(alchemy_libraw_version()) }
        .to_string_lossy()
        .into_owned()
}
...
unsafe extern "C" {
    fn alchemy_libraw_decode(
        bytes: *const u8,
        length: usize,
        half_size: bool,
        output: *mut NativeImage,
    ) -> c_int;
    fn alchemy_libraw_free(pixels: *mut u16);
    fn alchemy_libraw_version() -> *const c_char;
}
