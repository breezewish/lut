...
// This file is pinned from LibRaw commit 0029e794. The color-matrix dot products
// below use explicit fused operations so native and WebAssembly builds perform
// the same IEEE-754 rounding. Keep all other code identical to upstream.
...
void LibRaw::convert_to_rgb_loop(float out_cam[3][4])
{
  ...
  else if (imgdata.idata.colors == 3)
  {
    for (img = imgdata.image[0], row = 0; row < S.height; row++)
    {
      for (col = 0; col < S.width; col++, img += 4)
      {
        out[0] = fmaf(out_cam[0][2], img[2],
                      fmaf(out_cam[0][0], img[0], out_cam[0][1] * img[1]));
        out[1] = fmaf(out_cam[1][2], img[2],
                      fmaf(out_cam[1][0], img[0], out_cam[1][1] * img[1]));
        out[2] = fmaf(out_cam[2][2], img[2],
                      fmaf(out_cam[2][0], img[0], out_cam[2][1] * img[1]));
  ...
  else if (imgdata.idata.colors == 4)
  {
    for (img = imgdata.image[0], row = 0; row < S.height; row++)
    {
      for (col = 0; col < S.width; col++, img += 4)
      {
        out[0] = fmaf(out_cam[0][3], img[3],
                      fmaf(out_cam[0][2], img[2],
                           fmaf(out_cam[0][0], img[0],
                                out_cam[0][1] * img[1])));
        out[1] = fmaf(out_cam[1][3], img[3],
                      fmaf(out_cam[1][2], img[2],
                           fmaf(out_cam[1][0], img[0],
                                out_cam[1][1] * img[1])));
        out[2] = fmaf(out_cam[2][3], img[3],
                      fmaf(out_cam[2][2], img[2],
                           fmaf(out_cam[2][0], img[0],
                                out_cam[2][1] * img[1])));
        img[0] = CLIP((int)out[0]);
        img[1] = CLIP((int)out[1]);
        img[2] = CLIP((int)out[2]);
