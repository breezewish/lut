...
fn exports_real_dng_to_rgb16_tiff_and_json() {
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_lutify"))
...
fn text_color_policy_controls_ansi() {
    let directory = tempfile::tempdir().unwrap();
    let always = Command::new(env!("CARGO_BIN_EXE_lutify"))
        .arg(root().join("tests/fixtures/linear.dng"))
    ...
    let never = Command::new(env!("CARGO_BIN_EXE_lutify"))
        .arg(root().join("tests/fixtures/linear.dng"))
    ...
    let error_always = Command::new(env!("CARGO_BIN_EXE_lutify"))
    ...
    let error_never = Command::new(env!("CARGO_BIN_EXE_lutify"))
        .arg(directory.path().join("missing.dng"))
