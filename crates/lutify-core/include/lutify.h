...
typedef enum LutifyStatus {
  ...
  LUTIFY_INVALID_WHITE_BALANCE = 6,
} LutifyStatus;
 ...
 * temperature and tint are relative controls within [-100, 100]; zero keeps
 * the camera's As Shot white balance.
...
LutifyRenderResult lutify_render_tiff_v2(const uint16_t *pixels,
                                         ...
                                         const uint8_t *cube, size_t cube_len);
