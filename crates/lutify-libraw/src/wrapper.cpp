...
int lutify_libraw_decode(const std::uint8_t *bytes, std::size_t length,
                          bool half_size, LutifyDecodedImage *output) {
  ...
  processor.imgdata.params.highlight = 2; // Match upstream RAW Alchemy's Blend mode.
...
void lutify_libraw_free(std::uint16_t *pixels) { std::free(pixels); }

const char *lutify_libraw_version() { return LibRaw::version(); }
}
