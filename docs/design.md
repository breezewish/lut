...
The browser uses one Dedicated Worker. It hosts a custom, single-threaded LibRaw WASM build and the `alchemy-core` WASM build. Commands are serialized. For preview, Rust owns the resampling coordinates and requests only contributing rows from LibRaw's half-size RGB16 image. The persistent renderer keeps only the resulting longest-edge-1600 RGB16 source and the selected parsed LUT, then the Worker destroys the short-lived LibRaw instance so its larger image, RAW copy, and processing state do not survive into interactive work or export. EV changes pass only the new scalar, while LUT changes parse and replace only the LUT. Neither rerender path copies the source image or decodes RAW again. Full-resolution export decodes on demand and runs sequentially. The Worker passes approximately 1 MB RGB16 source views into a stateful Rust WASM encoder, so the generated binding never copies the complete decoded image into the color WASM. Color processing feeds each bounded quantized strip directly to a streaming Deflate TIFF writer. Batch TIFFs enter a pass-through ZIP incrementally, avoiding redundant compression and contiguous archive copies while retaining the final Blob chunks required by portable browser downloads.
...
- Full-resolution export holds one JS-owned decoded RGB16 image, one encoded output, and bounded source and quantized strip buffers at a time; no second full-image RGB16 allocation crosses into the color WASM.
