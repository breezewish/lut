...
The browser uses one Dedicated Worker. It hosts a custom, single-threaded LibRaw WASM build and the `alchemy-core` WASM build. Commands are serialized. The worker transfers one half-size RGB16 preview and the selected parsed LUT into a persistent Rust renderer, then destroys the short-lived LibRaw instance so its RAW copy and processing state do not survive into interactive work or export. EV changes pass only the new scalar, while LUT changes parse and replace only the LUT. Neither rerender path copies the source image or decodes RAW again. Full-resolution export decodes on demand and runs sequentially. The Worker passes approximately 1 MB RGB16 source views into a stateful Rust WASM encoder, so the generated binding never copies the complete decoded image into the color WASM. Color processing feeds each bounded quantized strip directly to a streaming Deflate TIFF writer. Batch TIFFs enter a pass-through ZIP incrementally, avoiding redundant compression and contiguous archive copies while retaining the final Blob chunks required by portable browser downloads.
