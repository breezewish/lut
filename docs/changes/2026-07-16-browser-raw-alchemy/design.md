...
- Preview LibRaw instances are short-lived. Rust requests only the half-size source rows that contribute to a longest-edge-1600 cache; the persistent renderer retains those sampled RGB16 pixels and releases the larger LibRaw image, RAW copy, and processing state.
