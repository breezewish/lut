...
Initial processing and interactive rerenders are progressive. An embedded JPEG is only an immediate labeled placeholder. The first processed comparison is longest-edge 384 and the settled comparison is longest-edge 1024. Interactive frames never approximate exposure or LUT color. The previous processed comparison remains visible until the next interaction frame atomically replaces it; an EV or LUT change never clears either canvas to a blank placeholder. Only the current recipe may update either canvas. Export remains unavailable until that current recipe has reached the settled resolution.
...
The acceptance fixture is the 6240 × 4168 Sony RAW in the repository, rendered by the production Chromium bundle. When available, its embedded JPEG must appear within 0.3 seconds. A cold selection must draw the longest-edge-1024 settled processed comparison within 2 seconds. Warm selections must draw their first longest-edge-384 processed comparison with p95 below 1 second and their settled comparison with p95 below 1.5 seconds. These boundaries begin in the file-selection handler and include file reading, Worker/WASM work, transfer, and Canvas drawing.
