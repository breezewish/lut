...
The workspace is a full-width, full-height editing application identified as LUTify in the document title and top toolbar, with persistent Light and Dark modes. It is flat, borderless, shadowless, neutral, and responsive. Desktop uses a top toolbar, dominant center comparison canvas, right adjustment inspector, and bottom photo filmstrip. The filmstrip omits a redundant photo count. Filmstrip and Look thumbnails preserve portrait and square framing instead of cropping every photo into a landscape tile; landscape tiles retain the compact 3:2 grid. Processing controls are hidden before a file is selected. The adjustment inspector shows a spinner whenever its visible EV or LUT recipe is not yet fully rendered; the canvas status reports the same state instead of reporting Ready. Camera and dimensions belong to the active document title. Output is visibly named and contains one format selector and one export action; progress is shown in that button and completion is announced in a toast. The output color-space assumption remains visible as a compact amber status in the toolbar and beside the export decision. When export is unavailable, that local status instead explains the current blocking reason. Desktop actions and fields use compact 30–36 px bodies; coarse-pointer and narrow-viewport actions retain at least 44 px targets. At narrow widths the canvas and inspector stack while the compact Output controls remain reachable below the internally scrolling Look catalog. Reduced-motion preferences are honored.
