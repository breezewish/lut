...
The application is a static project site. One Vite base path applies to the entry bundle, Worker, WASM, manifest, and LUT assets, so the same build model works at an origin root or a repository subpath. Rust, lightweight web checks, and browser production tests run independently. The browser job tests the repository-path bundle and uploads that exact output as an immutable Pages artifact. A main-branch hardware gate uses GitHub OIDC to start one SSM-managed NVIDIA T4 instance, checks out the exact commit, runs the WebGPU correctness and performance suite, and always stops the instance. A `main` deployment waits for every verification job, including that hardware gate, and never rebuilds or commits generated output.
