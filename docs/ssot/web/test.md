...
- White Balance exposes As Shot, Temperature, Tint, and one reset; its blue-to-amber and green-to-magenta sliders update the WebGPU preview, Look thumbnails, and export without another RAW decode.
- Temperature and Tint apply to every selected photo, identify mixed selection values, and reset both axes to exact As Shot.
...
- Output is visibly named, keeps the unverified color-space assumption beside the export decision, explains why its action is unavailable, defaults to 16-bit TIFF, switches to Quality 95 JPEG through its labeled format selector, and keeps its single action disabled while the active RAW is decoding or its visible EV/white-balance/LUT recipe is waiting to render.
...
- SwiftShader CI runs the independent corrected-v2 Preview pixel oracle, exact tiled AAHD fixtures across Bayer phases and seams, and a bounded packed Bayer DNG through the complete production WebGPU AAHD, color, TIFF, and download path against the native oracle with a six-code ceiling.
- The SwiftShader production export applies nonzero Temperature and Tint on WebGPU and matches the native Rust pipeline using the same relative recipe.
- Full-resolution real-camera production alignment remains an opt-in non-fallback hardware suite with a two-code ceiling; one command covers the seven-camera matrix, Sony and Leica AAHD cases, auto white balance, rotated fallback geometry, X-Trans shader parity, and hardware Preview.
- A main-branch GitHub Actions release gate runs the Nikon decoder boundary, production browser journeys, and portable SwiftShader correctness gates before Pages deployment, without cloud-provider credentials or external runners.
...
- An unchanged CC0 Sigma DP1 X3F decodes, reaches the ready editor state, and exports a nontrivial TIFF.
- An unchanged CC0 GoPro HERO7 GPR opens a concise English modal that explains its unsupported VC-5 compression and DNG conversion.
- A deterministic DNG using JPEG XL compression opens a concise English modal that explains compatible DNG conversion and the iPhone's JPEG Lossless (Most Compatible) setting.
...
- At mobile width with a processed RAW, the inspector scrolls without clipping adjustments or Looks, and Output keeps its title, color-space status, format selector, and 44px export action together when reached.
