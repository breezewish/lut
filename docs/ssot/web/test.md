...
- The self-labeled split export button has no repeated section heading, explains why its action is unavailable, defaults to 16-bit TIFF, switches to Quality 95 JPEG through its trailing menu, and keeps export disabled while the active RAW is decoding or its visible EV/white-balance/LUT recipe is waiting to render.
...
- At mobile width with a processed RAW, the inspector scrolls without clipping adjustments or Looks, and the 44px split export button remains reachable below the Look catalog.
