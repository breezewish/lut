...
- An opt-in hardware Preview test verifies a non-fallback WebGPU adapter, requires the first EV response at 1024px, compares every Base sample and every built-in Look's RGBA8 samples with the CPU Preview within one code, and records GPU execution plus readback time.
