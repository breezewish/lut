...
- The native/WASM decoder parity harness proves separated LibRaw RGB16 slices share the same WASM memory and out-of-bounds views fail; synthetic LinearRaw and real-CFA half-size outputs are sample-exact, while full-size real-CFA output is sample-exact inside the image and permits only one bounded AAHD outer-edge pixel.
