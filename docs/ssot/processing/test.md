...
- The native/WASM decoder parity harness covers every manifest fixture in full-size and half-size modes, including synthetic linear and lossy DNG, real Leica CFA DNG, and full-resolution Sony ARW; every case proves separated RGB16 slices share the same WASM memory and out-of-bounds views fail, all samples match exactly except for one bounded outer-edge pixel in full-size Leica AAHD output, and the Leica interior remains exact.
